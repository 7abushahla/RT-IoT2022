...
#include <math.h>
#include <time.h>
#include "treelite_wrap.h"

#define INPUT_DIM  94
#define NUM_ITERS  10000
#define BATCH      1        // bump (e.g., 50/100) if you want even smoother timing

// Keep const since the wrapper takes const float*
...
int main(void) {
  // Warm-up (stabilize caches/branch prediction)
  for (int k = 0; k < 5; ++k) {
    sink_cls = lgbm_predict_class_f32(Xf, INPUT_DIM);
  }

  printf("LGBM timing (RPi) with batching + baseline subtraction\n");
  ...
  for (int i = 0; i < NUM_ITERS; ++i) {
    ...
    if (inf_ms < 0.0) inf_ms = 0.0;      // guard tiny negatives from noise
  ...
  // Stats
  double avg = total / (double)NUM_ITERS;
  double ssd = 0.0;
  for (int i = 0; i < NUM_ITERS; ++i) {
    double d = (double)inference_times[i] - avg;
    ssd += d * d;
  }
  ...
  // Single labeled prediction (class only, matching wrapper signature)
