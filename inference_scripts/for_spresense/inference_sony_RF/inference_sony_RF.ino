...
// Measure cost of the timing loop without the model call
static inline float measure_baseline_ms() {
  unsigned long t0 = micros();
  for (int b = 0; b < BATCH; ++b) {
    // do nothing; keep same loop structure
    asm volatile("" ::: "memory");
  }
...
void setup() {
  ...
  for (int i = 0; i < INPUT_DIM; ++i)
    Xi16[i] = (int16_t)lrintf(Xf[i] * SCALE);

  // Warm-up a few times
  for (int k = 0; k < 5; ++k) sink_cls = rf_model_predict(Xi16, INPUT_DIM);

  Serial.println("RF timing with batching + baseline subtraction");

  // Baseline (loop + micros() overhead)
  float baseline_ms = measure_baseline_ms();

  float total_time = 0.0f;
  for (int i = 0; i < NUM_ITERS; ++i) {
    // time BATCH inferences at once
    unsigned long t0 = micros();
    for (int b = 0; b < BATCH; ++b) {
    ...
    // Per-inference time (ms), subtract baseline/BATCH
    float window_ms = (t1 - t0) / 1000.0f;
    float inf_time = (window_ms - baseline_ms) / (float)BATCH;
    if (inf_time < 0) inf_time = 0;  // guard against tiny negative from noise
    ...
    if (((i + 1) % 100) == 0) {
      ...
      Serial.print(" - Inference Time: "); Serial.print(inf_time, 6); Serial.println(" ms");
    }
  }

  // Stats
  float avg = total_time / NUM_ITERS;
  float ssd = 0.0f;
  for (int i = 0; i < NUM_ITERS; ++i) {
    float d = inference_times[i] - avg; ssd += d * d;
  }
  float stddev = (NUM_ITERS > 1) ? sqrtf(ssd / (NUM_ITERS - 1)) : 0.0f;
  float fps = (avg > 0) ? 1000.0f / avg : 0.0f;
  float fps_std = (avg > 0) ? (1000.0f / (avg * avg)) * stddev : 0.0f;
