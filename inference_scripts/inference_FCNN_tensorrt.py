...
time_csv_file = "tensorrt_inference_times.csv"
power_csv_file = "tensorrt_power_monitoring.csv"
...
ap.add_argument("--model", type=str, default="FCNN.trt", help="Name of the TensorRT model (with .trt extension)")
...
ap.add_argument("--sample_idx", type=int, default=5, help="Sample index to use for inference")
...
def classify(input_data, context, inputs, outputs, stream):
    ...
    context.set_tensor_address(outputs[0][2], int(outputs[0][1]))
    ...
    preds = outputs[0][0]
    # Ensure output is 2D for downstream argmax(axis=1)
...
print("=== TensorRT FCNN Model Inference ===")
...
print(f"Output tensor: {output_names[0]}")
...
preds = classify(sample, context, inputs, outputs, stream)
